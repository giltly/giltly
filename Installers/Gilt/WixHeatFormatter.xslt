...
<xsl:stylesheet version="1.0"
	...
	<!-- Set the MainAppExe -->
    <xsl:template match="wix:File[@Source = '$(var.GiltlyPackageDir)\bin\gilt.bacon.dll']">	
		<xsl:copy>            									
			<xsl:attribute name="Id">MainAppExe</xsl:attribute>	
			<xsl:attribute name="KeyPath">yes</xsl:attribute>	
			<xsl:attribute name="ProcessorArchitecture">$(var.Platform)</xsl:attribute>
			<xsl:attribute name="Source">$(var.GiltlyPackageDir)\bin\gilt.bacon.dll</xsl:attribute>
		</xsl:copy>	
    </xsl:template>		
